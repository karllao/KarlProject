...
#白名单设置
myphone=white_list.USER_WHITELIST
...
def main(config_data=None, aes_key_str=None):
    """
    主函数 - 支持两种调用方式：
    1. 直接调用：main(config_data, aes_key_str)
    2. 命令行调用：python main.py --config '{"USER":"xxx","PWD":"yyy",...}' [--aes-key 'key']
    
    Args:
        config_data: 配置字典或JSON字符串，包含 USER, PWD, MIN_STEP, MAX_STEP 等
        aes_key_str: AES密钥（可选），16个字符的字符串
    """
    global time_bj, encrypt_support, user_tokens, PUSH_PLUS_TOKEN, PUSH_PLUS_HOUR, PUSH_PLUS_MAX
    global sleep_seconds, users, passwords, min_step, max_step, use_concurrent, aes_key, config
    ...
    if aes_key_str is not None:
        aes_key = aes_key_str.encode('utf-8') if isinstance(aes_key_str, str) else aes_key_str
        if len(aes_key) == 16:
            encrypt_support = True
            user_tokens = prepare_user_tokens()
        else:
            print(f"警告：AES_KEY长度为 {len(aes_key)}，不是16字节，无法使用加密保存功能")
    
    # 优先使用环境变量（GitHub Actions模式）
    if config_data is None:
        if os.environ.get("CONFIG") is not None:
            ...
            print("错误：未提供CONFIG配置")
            print("使用方式：")
            print("  1. 直接调用：main(config_data={'USER':'xxx','PWD':'yyy',...})")
            print("  2. 命令行：python main.py --config '{\"USER\":\"xxx\",\"PWD\":\"yyy\",...}'")
            exit(1)
    
    # 处理配置数据
    if isinstance(config_data, str):
        try:
            config = dict(json.loads(config_data))
        except:
            print("CONFIG格式不正确，请检查JSON格式：使用双引号包裹字段和值")
            traceback.print_exc()
            exit(1)
    elif isinstance(config_data, dict):
        config = config_data
    else:
        print("错误：config_data 必须是字典或JSON字符串")
        exit(1)
    ...
    min_step = get_int_value_default(config, 'MIN_STEP', 8888)
    max_step = get_int_value_default(config, 'MAX_STEP', 11111)
    if users is None or passwords is None:
        print("错误：未正确配置账号密码")
        exit(1)
    if min_step ==8888 and max_step ==11111:
...
def entrance(data):
    print(data)
    event_user = data["number"]
    password = data["password"]
    step = data["step"]
    if event_user in myphone:
        config = {
            "USER": event_user,
            "PWD": password,
            "MIN_STEP": step,
        ...
        # AES 密钥必须是16个字符
        aes_key = "1q2w3e4r5a6s7d8f"
        result = main(config_data=config, aes_key_str=aes_key)
        # 处理返回结果,单账号所以取第一个
        if result[0]['success'] is True:
            message = {'msg':{"code":200,"msg":result}}
        else:
            message = {'msg':{"code":500,"msg":result}}
        return message
    else:
        print('非白名单用户，结束')
        return {'msg':{"code":412,"msg":"该账号不是老鱼的尊贵用户哦-_-"}}


if __name__ == "__main__":
    # 解析命令行参数
    parser = argparse.ArgumentParser(
        description='小米运动自动刷步数工具',
...
示例用法：
  ...
  # 带AES密钥
  python main.py --config '{"USER":"user@example.com","PWD":"password",...}' --aes-key '1234567890123456'
  ...
  python main.py --config-file config.json
    ...
    parser.add_argument(
        '--config',
        type=str,
        help='JSON格式的配置字符串，包含USER、PWD等字段'
    )
    parser.add_argument(
        '--config-file',
        type=str,
    ...
    if args.config_file:
        try:
            ...
            print(f"已从文件加载配置：{args.config_file}")
