...
#白名单设置
myphone=['18814117895','18811112222','7777777@qq.com']
...
def login(user,password):
    user_phone = None
    if len(user) == 11 and user.isdigit():
        # 判断是邮箱还是号码
        user_phone = "+86" + user
    else:
        user_phone = user
    url1 = "https://api-user.huami.com/registrations/" + user_phone + "/tokens"
    headers = {
        "Content-Type":"application/x-www-form-urlencoded;charset=UTF-8",
    "User-Agent":"MiFit/5.3.0 (iPhone; iOS 14.0.1; Scale/2.00)"
        }
    data1 = {
        "client_id":"HuaMi",
        "password":f"{password}",
    ...
    if len(user) == 11 and user.isdigit():
        ...
        data2 = {
            ...
            "code":f"{code}",
            "country_code":"CN",
            "device_id":"2C8B4229-0CCD-4E94-8CBA-CB8EA6E622A1",
            "device_model":"phone",
...
def main(user, passwd, step):
    ...
    step = str(step)
    if step == '':
        print ("设置为随机步数（26000-36000）")
        step = str(random.randint(26000,36000))
        print("设置随机步数为：" + step)
    login_token = 0
    login_token,userid = login(user,password)
    if login_token == 0:
        print("登陆失败！")
        return {'msg':{"code":401,"result":"login fail","msg":"登录失败，请稍后再试！","time":now}}
    ...
    yesterday_time = str(int(time.mktime(time.strptime(str(yesterday), '%Y-%m-%d'))))
    #print (yesterday_time)
    data = f'userid={userid}&last_sync_data_time={yesterday_time}&device_type=0&last_deviceid=DA932FFFFE8816E7&data_json={data_json}'
    
    response = requests.post(url, data=data, headers=head).json()
    #print(response)
    #result = f"{user[:4]}****{user[-4:]}: [{now}] 修改步数（{step}）"+ response['message']
    result = now + " 修改步数: "+ step + ",修改结果： " + response['message']
    print(result)
...
def get_time():
    ...
    # 转换为毫秒级时间戳
    current_timestamp_ms = str(current_timestamp * 1000)
...
def entrance(data):
    ...
    if event_user in myphone:
        result = main(event_user,password,step)
        return result
    else:
        print('非白名单用户，结束')
